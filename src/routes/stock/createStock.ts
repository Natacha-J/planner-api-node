...
import { StockIngredientsInstance, StockInstance } from "../../types/modelsType"
...
module.exports = (app: Express) => {
    app.post('/api/stocks/:id', auth, (req: Request, res: Response) => {
        StockIngredients.findAll({
            where: {
                StockId: req.params.id
            }
        })
        .then((listIngredients: StockIngredientsInstance[]) => {
            //remove stockList's ingredient not in user's request
            listIngredients.map((ingredientInList: StockIngredientsInstance) => {
            ...
            req.body.ingredients.map((ingredientInRequest: StockIngredientsInstance) => {
